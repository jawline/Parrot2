...
let out_article_path article output_base =
  sprintf "%s/article/%s.html" output_base (out_article_name article.name)
;;

let out_tags_path tag_name output_base = sprintf "%s/tags/%s.html" output_base (out_article_name tag_name)
;;

let emit_tag tag_name tag tags_template tag_template output_folder ~zone =
  Out_channel.write_all
  (out_tags_path tag_name output_folder)
  ~data:(render_tag tag_name tag ~tags_template:tags_template ~tag_template:tag_template ~zone)
;;

let emit_article article article_template output_folder ~zone =
  Out_channel.write_all
    (out_article_path article output_folder)
    ~data:(render_article article ~template:article_template ~zone)
;;
...
let clean_build input_directory =
  ...
  let rec emit_tags = function
    ...
    | (tag_name, tag_items) :: xs ->
      ...
      emit_tag tag_name tag_items tags_template tag_template output_directory ~zone:timezone;
      emit_tags xs
  in
  ...
  printf "Finished. Output in %s" output_directory;
;;
